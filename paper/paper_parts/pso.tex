...
In our formulation, the position of each particle \(s \in [1,\swarmsize]\) is represented by a matrix \(P_s\) with four rows and \(\nmax\) columns, where \(\nmax\) denotes the maximum number of available fixtures. Each column of \(P_s\) represents a fixture, while each row corresponds to one of the decision variables:
\begin{itemize}
