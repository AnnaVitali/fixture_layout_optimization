...
def generate_figure_2d_1d(rect1_verts, rect2_verts, title_suffix, xlim_fixed=None, ylim_fixed=None):
    ...
    ax_2d.add_patch(Polygon(rect1_verts, closed=True, facecolor=RECT1_GRAY, alpha=0.6, edgecolor='black', linewidth=1.2))
    ax_2d.add_patch(Polygon(rect2_verts, closed=True, facecolor=RECT2_GRAY, alpha=0.6, edgecolor='black', linewidth=1.2))
    ...
    bottom_y = -0.40
    overlap_y = -0.08
    ax_xproj.add_patch(Rectangle((min1x - x_pad, top_y), (max1x - min1x) + 2 * x_pad, proj_height, facecolor=RECT1_GRAY, edgecolor='black', alpha=0.9, linewidth=1.0, label='Rect 1'))
    ax_xproj.add_patch(Rectangle((min2x - x_pad, bottom_y), (max2x - min2x) + 2 * x_pad, proj_height, facecolor=RECT2_GRAY, edgecolor='black', alpha=0.9, linewidth=1.0, label='Rect 2'))
    overlap_min_x = max(min1x - x_pad, min2x - x_pad)
    overlap_max_x = min(max1x + x_pad, max2x + x_pad)
    if overlap_min_x < overlap_max_x:
        ax_xproj.add_patch(Rectangle((overlap_min_x, overlap_y), overlap_max_x - overlap_min_x, proj_height, facecolor=OVERLAP_GRAY, edgecolor='black', alpha=0.95, linewidth=1.0, label='Overlap'))
    ...
    ax_yproj.add_patch(Rectangle((min1y - y_pad, top_y), (max1y - min1y) + 2 * y_pad, proj_height, facecolor=RECT1_GRAY, edgecolor='black', alpha=0.9, linewidth=1.0, label='Rect 1'))
    ax_yproj.add_patch(Rectangle((min2y - y_pad, bottom_y), (max2y - min2y) + 2 * y_pad, proj_height, facecolor=RECT2_GRAY, edgecolor='black', alpha=0.9, linewidth=1.0, label='Rect 2'))
    overlap_min_y = max(min1y - y_pad, min2y - y_pad)
    overlap_max_y = min(max1y + y_pad, max2y + y_pad)
    if overlap_min_y < overlap_max_y:
        ax_yproj.add_patch(Rectangle((overlap_min_y, overlap_y), overlap_max_y - overlap_min_y, proj_height, facecolor=OVERLAP_GRAY, edgecolor='black', alpha=0.95, linewidth=1.0, label='Overlap'))
