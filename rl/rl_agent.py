...
WORKPIECE_CONSTRAINTS = {
    ...
    'door': {'min_fixtures': 10, 'max_fixtures': 20},
    'door_porthole': {'min_fixtures': 10, 'max_fixtures': 20},
}
...
class FixtureLayoutEnv(gym.Env):
    ...
    def _preprocess_valid_grid_positions(self) -> set:
        ...
        print(f"[DEBUG] Workpiece bounds: minx={minx}, maxx={maxx}, miny={miny}, maxy={maxy}")
        print(f"[DEBUG] Grid range x: [{self.x_positions[0]}, {self.x_positions[-1]}], count={len(self.x_positions)}")
        print(f"[DEBUG] Grid range y: [{self.y_positions[0]}, {self.y_positions[-1]}], count={len(self.y_positions)}")
        ...
        print(f"[DEBUG] Positions rejected by bounds check: {rejected_by_bounds}")
        print(f"[DEBUG] Positions rejected by hole intersection: {rejected_by_holes}")
        print(f"[DEBUG] Positions accepted as safe: {accepted}")
