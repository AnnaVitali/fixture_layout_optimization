...
# Dimensionless reward-shaping weights.
# The moment-of-inertia improvement is deliberately dominant; the other
# terms only shape the construction process.
REWARD_MOI_WEIGHT = 5.0
REWARD_COUNT_WEIGHT = 0.10
REWARD_DISTANCE_WEIGHT = 0.05
REWARD_MIN_FIXTURES_BONUS = 0.25
REWARD_INVALID_ACTION_PENALTY = -1.0
REWARD_INCOMPLETE_LAYOUT_PENALTY = -0.50
...
def get_valid_actions(
    ...
    The Q-learning state deliberately abstracts away fixture geometry, but the
    feasible action set does depend on geometry.  Therefore the action cache
    must *not* be keyed only by the abstract RL state.  We include the actual
    fixture configuration in the cache key so that two layouts with the same
    (count, availability) state cannot incorrectly share feasible actions.
    ...
    geometry_key = tuple(
        (
            ...
            round(float(f.x), 6),
            round(float(f.y), 6),
            round(float(f.angle), 6),
        )
        for f in env.fixtures
    )
    env_key = (
        geometry_key,
        env.fixture_availability[1],
        env.fixture_availability[2],
    )
...
def place_fixture_directly(
    ...
    """
    Place a fixture directly at a grid position without going through env.step().

    Reward design
    -------------
    All reward components are dimensionless and kept on comparable scales:

      1. Main objective: normalized increase in the system moment of inertia.
      2. Count shaping: incremental change in (n / Nmax)^2.
      3. Distance shaping: average distance to previous fixtures, normalized by
         the workpiece diagonal and Nmax.
      4. Feasibility bonus: awarded once, exactly when Nmin is reached.

    This avoids the 1e6--1e10 coefficients used previously and keeps the
    learning signal aligned with the final objective (moment of inertia).
    ...
    if fixture_type not in [1, 2]:
        return REWARD_INVALID_ACTION_PENALTY, False

    if env.fixture_availability[fixture_type] <= 0:
        return REWARD_INVALID_ACTION_PENALTY, False

    max_fixtures = MAXIMUM_FIXTURES.get(workpiece_name, 6)
    if num_fixtures_before >= max_fixtures:
        return REWARD_INVALID_ACTION_PENALTY, False
    ...
    # Keep a snapshot of the previous fixtures for the distance term.
    previous_fixtures = list(env.fixtures)

    old_moment = float(env.cumulative_moment)

    env.fixtures.append(fixture_state)
    env.fixture_availability[fixture_type] -= 1

    new_moment = float(env._compute_system_moment_of_inertia(env.fixtures))
    env.cumulative_moment = new_moment

    num_fixtures_after = len(env.fixtures)
    delta_moi = new_moment - old_moment

    # Characteristic geometric scale for MOI: A * L^2.
    # Do not multiply by Nmax: over a complete episode, sum(delta_moi) already
    # telescopes to the final MOI, so this keeps the physical objective dominant.
    workpiece_width = float(env.visualizer.max_x - env.visualizer.min_x)
    workpiece_height = float(env.visualizer.max_y - env.visualizer.min_y)
    workpiece_diagonal = max(1.0, float(np.hypot(workpiece_width, workpiece_height)))

    max_fixture_area = max(
        float(FIXTURE_DIMENSIONS[t][0] * FIXTURE_DIMENSIONS[t][1])
        for t in (1, 2)
    )
    moi_reference = max(1.0, max_fixture_area * workpiece_diagonal**2)

    # 1) Main optimization objective.
    normalized_moi_gain = delta_moi / moi_reference

    # 2) Mild preference for using more fixtures.
    # Use an incremental potential difference, not n_after^2 at every step.
    normalized_count_gain = (
        (num_fixtures_after / max_fixtures) ** 2
        - (num_fixtures_before / max_fixtures) ** 2
    )

    # 3) Mild preference for spatially distributed fixtures.
    # Dividing by Nmax bounds the *cumulative* contribution across an episode.
    normalized_distance = 0.0
    if num_fixtures_before > 0:
        total_distance = 0.0
        for prev_fixture in previous_fixtures:
            ...
            total_distance += float(
                np.hypot(x_pos - prev_center_x, y_pos - prev_center_y)
            )

        avg_distance = total_distance / num_fixtures_before
        normalized_distance = (
            avg_distance / workpiece_diagonal
        ) / max_fixtures

    # 4) Award the Nmin feasibility bonus once, on threshold crossing only.
    min_fixtures = MINIMUM_FIXTURES.get(workpiece_name, 3)
    reached_minimum_now = (
        num_fixtures_before < min_fixtures <= num_fixtures_after
    )
    minimum_bonus = 1.0 if reached_minimum_now else 0.0

    reward = (
        REWARD_MOI_WEIGHT * normalized_moi_gain
        + REWARD_COUNT_WEIGHT * normalized_count_gain
        + REWARD_DISTANCE_WEIGHT * normalized_distance
        + REWARD_MIN_FIXTURES_BONUS * minimum_bonus
    )
...
def train_episode(
    ...
    The episode terminates when Nmax is reached, when the environment reaches
    max_steps, when no feasible actions remain, or when a placement fails.
    A small terminal penalty is applied if the episode ends before Nmin.
    ...
    while not done:
        ...
        if num_current_fixtures >= max_fixtures:
            break
        ...
        reward, success = place_fixture_directly(
            env,
            x_idx,
            y_idx,
            fixture_type,
        ...
        next_valid_actions = get_valid_actions(env, agent) if success else []
        num_current_fixtures = len(env.fixtures)

        reached_nmax = num_current_fixtures >= max_fixtures
        reached_step_limit = env.step_count >= env.max_steps
        no_more_actions = not next_valid_actions
        ...
        # If construction gets stuck before the minimum number of fixtures,
        # put that information into the actual TD target rather than adding a
        # huge post-hoc bonus/penalty after Q-learning has finished.
        if done and num_current_fixtures < min_fixtures:
            reward += REWARD_INCOMPLETE_LAYOUT_PENALTY
...
def evaluate_episode(
    ...
    The original implementation used 30% exploratory actions during evaluation;
    that behavior is preserved here.  The returned reward uses exactly the same
    normalized reward as training, with no additional post-hoc completion bonus.

    Returns:
        (cumulative_reward, num_fixtures_placed, final_moi, fixtures_config)
    """
    ...
    while not done:
        ...
        if np.random.random() < 0.3:  # Preserved from the original code
        ...
        reached_step_limit = env.step_count >= env.max_steps
        no_more_actions = not next_valid_actions
        done = reached_nmax or reached_step_limit or no_more_actions
