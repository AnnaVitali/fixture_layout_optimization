package workpiece.doorPorthole;
...
public class DoorPorthole implements Workpiece {

    private static final List<Tuple<Float, Float>> HOLES = List.of(
            new Tuple<>(950f,  725f),
            new Tuple<>(990f, 727f),
            new Tuple<>(290f, 260f)
    );
    private static final List<Float> HOLESWIDTH = List.of(
            40f, 44f, 280f
    );
    private static final List<Float> HOLESHEIGHT = List.of(
            40f, 44f, 280f
    );
    ...
    private void defineWorkpieceInequalityAndVertices(){
        ...
        PolygonGeometry.LineSegment line1 = PolygonGeometry.lineEquationFromPoints(new Tuple<>(0.0, 0.0), new Tuple<>(0.0, 800.0));
        lineEquations.add(line1);

        PolygonGeometry.LineSegment line2 = PolygonGeometry.lineEquationFromPoints(new Tuple<>(0.0, 800.0), new Tuple<>(2000.0, 800.0));
        lineEquations.add(line2);

        PolygonGeometry.LineSegment line3 = PolygonGeometry.lineEquationFromPoints(new Tuple<>(2000.0, 800.0), new Tuple<>(2000.0, 0.0));
        lineEquations.add(line3);

        PolygonGeometry.LineSegment line4 = PolygonGeometry.lineEquationFromPoints(new Tuple<>(2000.0, 0.0), new Tuple<>(0.0, 0.0));
        lineEquations.add(line4);

        Tuple<Float, Float> center = PolygonGeometry.computePolygonCentroid(List.of(
                new Tuple<>(0.0f, 0.0f),
                new Tuple<>(0.0f, 800.0f),
                new Tuple<>(2000.0f, 800.0f),
                new Tuple<>(2000.0f, 0.0f)
        ));
