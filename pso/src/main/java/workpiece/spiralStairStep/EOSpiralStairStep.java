...
public class EOSpiralStairStep {
    
    private static final String EXPERT_FILE_PATH = "./resources/expert/expert_operator_spiral_stair_step.json";
    private static final String RESULT_FILE_PATH = "./resources/pso/pso_eo_spiral_stair_step.json";


    public static void main(String[] args) {
        String content;
        try {
            content = new String(Files.readAllBytes(Paths.get(EXPERT_FILE_PATH)));
        } catch (IOException e) {
            throw new RuntimeException(e);
        }

        SpiralStairStep spiralStairStep = new SpiralStairStep();

        NoOverlapConstraint noOverlapConstraint = new NoOverlapConstraint(spiralStairStep.getHoles());
        GeometryConstraint geometryConstraint = new GeometryConstraint(spiralStairStep.getInequalities(), spiralStairStep.getVertices().stream().toList(),
        ...
        ParticleSwarmOptimization pso = new ParticleSwarmOptimization(
                ...
                noOverlapConstraint,
                fixtureConstraints,
                spiralStairStep.getMinFix(),
                spiralStairStep.getMaxFix()
        );
