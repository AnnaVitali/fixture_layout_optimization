...
public class Speaker implements Workpiece {
    ...
    private void defineWorkpieceInequalityAndVertices(){
        ...
        PolygonGeometry.LineSegment line1 = PolygonGeometry.lineEquationFromPoints(new Tuple<>(0.0, 0.0), new Tuple<>(0.0, 360.0));
        lineEquations.add(line1);

        PolygonGeometry.LineSegment line2 = PolygonGeometry.lineEquationFromPoints(new Tuple<>(0.0, 360.0), new Tuple<>(600.0, 360.0));
        lineEquations.add(line2);

        PolygonGeometry.LineSegment line3 = PolygonGeometry.lineEquationFromPoints(new Tuple<>(600.0, 360.0), new Tuple<>(600.0, 0.0));
        lineEquations.add(line3);

        PolygonGeometry.LineSegment line4 = PolygonGeometry.lineEquationFromPoints(new Tuple<>(600.0, 0.0), new Tuple<>(0.0, 0.0));
        lineEquations.add(line4);

        Tuple<Float, Float> center = PolygonGeometry.computePolygonCentroid(List.of(
                new Tuple<>(0.0f, 0.0f),
                new Tuple<>(0.0f, 360.0f),
                new Tuple<>(600.0f, 360.0f),
                new Tuple<>(600.0f, 0.0f)
        ));
