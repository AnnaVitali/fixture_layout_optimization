import optuna
import subprocess
import re
import logging

def objective(trial):
    # swarmSize = trial.suggest_int('swarmSize', 100, 2000)
    # maxIters = trial.suggest_int('maxIters', 100, 3000)
    # inertias = trial.suggest_float('inertias', 0.1, 1.0)
    # c1s = trial.suggest_float('c1s', 0.1, 3.0)
    # c2s = trial.suggest_float('c2s', 0.1, 3.0)

    populationSize = trial.suggest_int('populationSize', 100, 2000)
    maxGenerations = trial.suggest_int('maxGenerations', 100, 3000)
    crossoverRate = trial.suggest_float('crossoverRate', 0.1, 1.0)
    differentialWeight = trial.suggest_float('differentialWeight', 0.1, 1.0)



    # Build the command to run the .jar
    # cmd = [
    #     'java', '-jar', '../target/pso-constraint-rotation-0.1.jar',
    #     #str(swarmSize), str(maxIters), str(minInertia), str(maxInertia), str(c1s), str(c2s)
    #     str(swarmSize), str(maxIters), str(inertias), str(c1s), str(c2s)
    # ]

    cmd = [
        'java', '-jar', '../target/pso-constraint-rotation-0.1.jar',
        #str(swarmSize), str(maxIters), str(minInertia), str(maxInertia), str(c1s), str(c2s)
        str(populationSize), str(maxGenerations), str(crossoverRate), str(differentialWeight)
    ]

    try:
        result = subprocess.run(cmd, capture_output=True, text=True, timeout=300)
    except subprocess.TimeoutExpired:
        logging.warning("Subprocess timed out for params: %s", trial.params)
        return float('inf')

    if result.returncode != 0:
        logging.warning("Jar exited with code %s. stderr: %s", result.returncode, result.stderr.strip())

    # Combine stdout and stderr, find last non-empty line
    combined = "\n".join([s for s in (result.stdout or "", result.stderr or "") if s is not None])
    lines = [line.strip() for line in combined.splitlines() if line.strip()]
    if not lines:
        logging.warning("No output to parse for params: %s", trial.params)
        return float('inf')

    last_line = lines[-1]
    # Extract a floating number (supports scientific notation)
    m = re.search(r'[-+]?\d*\.?\d+(?:[eE][-+]?\d+)?', last_line)
    if not m:
        logging.warning("No numeric value found in last line: %s", last_line)
        return float('inf')

    try:
        fitness = float(m.group(0))
    except ValueError:
        logging.warning("Failed to convert extracted token to float: %s", m.group(0))
        return float('inf')

    return fitness

if __name__ == '__main__':

    study = optuna.create_study(direction='minimize')
    study.optimize(objective, n_trials=100)

    print('Best hyperparameters:', study.best_params)
